...
use crate::{client::{Client, ManagementClient}, views::{QueuesView, OverviewView}};
...
pub struct App
{
    ...
    pub overview: OverviewView<Client>,
}
...
impl App
{
    pub fn new() -> Self {
        ...
        let queues = QueuesView::new(client.clone(), queue_rx);
        let overview = OverviewView::new(client.clone(), overview_rx);
    
        thread::spawn(move || {
            loop {
                let queues_info = thread_client.get_queues_info();
                let overview = thread_client.get_overview();
    
                let _ = queue_tx.send(queues_info);
                let _ = overview_tx.send(overview);
